import os
import io

# Redirect Blender's configuration, data, and scripts to your custom paths
os.environ["BLENDER_USER_CONFIG"] = r""
os.environ["BLENDER_USER_SCRIPTS"] = r""
os.environ["BLENDER_USER_DATAFILES"] = r""

import bpy
import sys
import warnings
import math
import winreg
import bmesh
import fabex

# Register the addon
if hasattr(fabex, "register"):
    fabex.register()
else:
    print("No register() function found in addon.")


def resolve_windows_font_path(display_name):
    """
    Given a display name like 'Arial' or 'Bahnschrift SemiLight SemiConde',
    this function returns the actual font file path.
    """
    display_name = display_name.lower()
    fonts_dir = os.path.join(os.environ["WINDIR"], "Fonts")

    try:
        with winreg.OpenKey(winreg.HKEY_LOCAL_MACHINE, r"SOFTWARE\Microsoft\Windows NT\CurrentVersion\Fonts") as key:
            for i in range(winreg.QueryInfoKey(key)[1]):
                name, font_file, _ = winreg.EnumValue(key, i)
                if display_name in name.lower():
                    return os.path.join(fonts_dir, font_file)
    except Exception as e:
        print("🛑 Registry access failed:", e)

    return None


def resolve_font_path(font_name):
    font_name = font_name.strip().lower()
    fonts_dir = os.path.join(os.environ["WINDIR"], "Fonts")

    try:
        with winreg.OpenKey(winreg.HKEY_LOCAL_MACHINE, r"SOFTWARE\Microsoft\Windows NT\CurrentVersion\Fonts") as key:
            for i in range(0, winreg.QueryInfoKey(key)[1]):
                display_name, font_file, _ = winreg.EnumValue(key, i)
                if font_name in display_name.lower():
                    return os.path.join(fonts_dir, font_file)
    except Exception as e:
        print(f"Registry access failed: {e}")

    return None


def setup_blender_scene(engrave_text, font_path, text_font_size, text_position_mm, rotation_degrees,
                        layer_height_mm, safe_height_mm, save_dir, feedrate_mm, spindle_rpm, final_height_mm, work_area_width, work_area_height, gap_distance_mm=0.2):
    blend_file_path = os.path.join(save_dir, "output.blend")
    # addon_name = "bl_ext.user_default.fabex"

    # Suppress warnings
    warnings.filterwarnings("ignore")

    # Capture print output and errors
    original_stdout = sys.stdout
    original_stderr = sys.stderr
    
    # Create StringIO objects to capture output
    stdout_capture = io.StringIO()
    stderr_capture = io.StringIO()
    
    sys.stdout = stdout_capture
    sys.stderr = stderr_capture

    def _recenter_geometry_and_origin(obj):
        """Recenters the object at the origin: X-axis at bounding box center, Y-axis at center of mass."""
        # Get bounding box for X centering
        bb = obj.bound_box
        min_x = min(v[0] for v in bb)
        max_x = max(v[0] for v in bb)
        cx = 0.5 * (min_x + max_x)

        # Center of mass for Y centering
        obj.select_set(True)
        bpy.context.view_layer.objects.active = obj
        bpy.ops.object.origin_set(type='ORIGIN_CENTER_OF_MASS', center='BOUNDS')

        # Now move the object to the calculated X and Y positions
        obj.location.x -= cx  # Set the X location to center it in the bounding box
        bpy.context.view_layer.update()

    try:
        # ---------- FABEX / Scene config ----------
        bpy.context.scene.render.engine = 'FABEX_RENDER'
        bpy.context.scene.interface.level = '2'
        bpy.context.scene.cam_machine.feedrate_default = 1.51

        # Units in millimeters
        bpy.context.scene.unit_settings.length_unit = 'MILLIMETERS'

        bpy.context.scene.cam_machine.output_tool_change = False
        bpy.context.scene.cam_machine.eval_splitting = False
        bpy.context.scene.cam_machine.working_area[0] = work_area_width / 1000.0
        bpy.context.scene.cam_machine.working_area[1] = work_area_height / 1000.0

        # Clean scene
        bpy.ops.object.select_all(action='SELECT')
        bpy.ops.object.delete(use_global=False)

        temp_placeholder = "___PIPE_PLACEHOLDER___"
        # Preserve literal '|' when user types <|>
        if '<|>' in engrave_text:
            engrave_text_processed = engrave_text.replace('<|>', temp_placeholder)
        else:
            engrave_text_processed = engrave_text

        # ----------------- Create text mesh -----------------
        depsgraph = bpy.context.evaluated_depsgraph_get()
        text_obj = None

        if '|' in engrave_text_processed:
            # Split mode (separate parts joined later)
            gap_distance = gap_distance_mm / 1000.0
            parts = [part.replace(temp_placeholder, '|') for part in engrave_text_processed.split('|')]

            created_objects = []
            bpy.ops.object.select_all(action='DESELECT')

            x_offset = 0.0
            for i, part in enumerate(parts):
                bpy.ops.object.text_add(location=(x_offset, 0.0, 0.0))
                obj_i = bpy.context.object
                obj_i.data.body = part
                obj_i.data.size = text_font_size / 1000.0
                obj_i.name = f"TextPart_{i}"

                # Apply font
                if font_path:
                    try:
                        vect_font = bpy.data.fonts.load(font_path)
                        obj_i.data.font = vect_font
                        print(f"Font '{font_path}' loaded for part {i}.")
                    except Exception as e:
                        print(f"Failed to load font '{font_path}' for part {i}: {e}")

                # Convert to mesh to get accurate width
                bpy.ops.object.convert(target='MESH')
                bpy.context.view_layer.update()

                eval_obj = obj_i.evaluated_get(depsgraph)
                width = float(eval_obj.dimensions.x)
                created_objects.append(obj_i)

                x_offset += width + gap_distance

            # Join all parts
            for o in created_objects:
                o.select_set(True)
            bpy.context.view_layer.objects.active = created_objects[0]
            bpy.ops.object.join()

            text_obj = bpy.context.object
            text_obj.name = "Text"

            bpy.context.view_layer.update()

            # Center geometry & origin for stable placement
            _recenter_geometry_and_origin(text_obj)

        else:
            # Single text object
            bpy.ops.object.text_add(location=(0.0, 0.0, 0.0))
            text_obj = bpy.context.object
            text_obj.data.size = text_font_size / 1000.0

            # Restore literal '|' if needed
            if '<|>' in engrave_text:
                text_obj.data.body = engrave_text_processed.replace(temp_placeholder, '|')
            else:
                text_obj.data.body = engrave_text_processed

            # Apply font
            if font_path:
                try:
                    vect_font = bpy.data.fonts.load(font_path)
                    text_obj.data.font = vect_font
                    print(f"Font '{font_path}' loaded.")
                except Exception as e:
                    print(f"Failed to load font '{font_path}': {e}")
            else:
                print("No font selected.")

            # Convert to mesh and center
            bpy.ops.object.convert(target='MESH')
            bpy.context.view_layer.update()

            # Give a predictable origin/center
            _recenter_geometry_and_origin(text_obj)

        # Dimensions (debug)
        print("Actual Dimensions After Conversion:", tuple(text_obj.dimensions))

        # ----------------- Place & rotate (no extra Y slide) -----------------
        text_pos_meters = (text_position_mm[0] / 1000.0,
                           text_position_mm[1] / 1000.0,
                           text_position_mm[2] / 1000.0)
        text_obj.location = text_pos_meters
        text_obj.rotation_euler = (0.0, 0.0, math.radians(rotation_degrees))

        # ----------------- CAM op -----------------
        bpy.ops.scene.cam_operation_add()
        bpy.ops.wm.save_as_mainfile(filepath=blend_file_path)

        camop = bpy.context.scene.cam_operations[0]
        camop.cut_type = 'ONLINE'
        camop.stepdown = layer_height_mm / 1000.0          # layer height
        camop.movement.free_height = safe_height_mm / 1000.0  # Safe height
        camop.outlines_count = 2
        camop.feedrate = feedrate_mm / 1000.0
        camop.spindle_rpm = spindle_rpm
        camop.output_trailer = True
        camop.gcode_trailer = f"G00 X0Y0Z{final_height_mm}"

        bpy.ops.object.calculate_cam_path()

        gcode_file_path = os.path.join(save_dir, "Op_Text_1.tap")
        print("COMPLETED! GCODE is generated", gcode_file_path)

        # Reset scene (optional)
        bpy.ops.wm.read_factory_settings(use_empty=True)

    finally:
        # Restore stdout/stderr and warnings
        captured_stdout = stdout_capture.getvalue()
        captured_stderr = stderr_capture.getvalue()

        stdout_capture.close()
        stderr_capture.close()

        sys.stdout = original_stdout
        sys.stderr = original_stderr
        warnings.resetwarnings()

        # If you want the logs, uncomment:
        # if captured_stdout:
        #     print("=== CAPTURED STDOUT ===")
        #     print(captured_stdout)
        #     print("=== END STDOUT ===")
        # if captured_stderr:
        #     print("=== CAPTURED STDERR ===")
        #     print(captured_stderr)
        #     print("=== END STDERR ===")

    return gcode_file_path


import numpy as np
import matplotlib.pyplot as plt
from mpl_toolkits.mplot3d import Axes3D


def build_vandermonde(X, Y, degree):
    """
    Create the design matrix (Vandermonde) for 2D polynomial fitting.
    """
    terms = []
    for i in range(degree + 1):
        for j in range(degree + 1 - i):
            terms.append((X ** i) * (Y ** j))
    return np.vstack(terms).T


def fit_polynomial_surface_numpy(points, degree=2):
    """
    Fit a polynomial surface using numpy only.

    :param points: Nx3 array of [X, Y, Z]
    :param degree: Degree of polynomial surface
    :return: Coefficients, meshgrid, Z_pred
    """
    points = np.array(points)
    X, Y, Z = points[:, 0], points[:, 1], points[:, 2]

    A = build_vandermonde(X, Y, degree)
    coeffs, *_ = np.linalg.lstsq(A, Z, rcond=None)  # least squares solution

    # Grid for plotting
    x_range = np.linspace(X.min(), X.max(), 50)
    y_range = np.linspace(Y.min(), Y.max(), 50)
    X_grid, Y_grid = np.meshgrid(x_range, y_range)

    A_grid = build_vandermonde(X_grid.ravel(), Y_grid.ravel(), degree)
    Z_pred = A_grid @ coeffs
    Z_grid = Z_pred.reshape(X_grid.shape)

    return coeffs, X_grid, Y_grid, Z_grid


def plot_surface(points, X_grid, Y_grid, Z_grid):
    fig = plt.figure()
    ax = fig.add_subplot(111, projection='3d')

    points = np.array(points)
    ax.scatter(points[:, 0], points[:, 1], points[:, 2], color='r', label='Data')
    ax.plot_surface(X_grid, Y_grid, Z_grid, cmap='viridis', alpha=0.6)

    ax.set_xlabel('X')
    ax.set_ylabel('Y')
    ax.set_zlabel('Z')
    ax.set_title("Fitted Polynomial Surface")
    plt.axis("equal")
    plt.legend()
    plt.show()


def calculate_z_from_poly(X, Y, coeffs):
    """
    Calculate Z values from X and Y using the polynomial coefficients.

    :param X: Array of X values
    :param Y: Array of Y values
    :param coeffs: Polynomial coefficients
    :return: Calculated Z values
    """
    degree = int(np.sqrt(len(coeffs))) - 1  # Assuming a square polynomial
    Z = np.zeros_like(X)
    index = 0
    for i in range(degree + 1):
        for j in range(degree + 1 - i):
            Z += coeffs[index] * (X ** i) * (Y ** j)
            index += 1
    return Z
